...
ControlPolyTwoRK3Tpl<Scalar>::ControlPolyTwoRK3Tpl(const std::size_t nu) : ControlAbstractTpl<Scalar>(nu, 3*nu) {}

template <typename Scalar>
ControlPolyTwoRK3Tpl<Scalar>::~ControlPolyTwoRK3Tpl() {}

template <typename Scalar>
void ControlPolyTwoRK3Tpl<Scalar>::resize(const std::size_t nu){
  nu_ = nu;
  np_ = 3*nu;
}

template <typename Scalar>
void ControlPolyTwoRK3Tpl<Scalar>::value(double t, const Eigen::Ref<const VectorXs>& p, Eigen::Ref<VectorXs> u_out) const {
  ...
  Scalar t2 = t*t;
  u_out = (Scalar(4.5)*t2-Scalar(1.5)*t)*p2 + 
          (-Scalar(9.)*t2+Scalar(6.)*t)*p1 + 
          (Scalar(4.5)*(t2 - t) + Scalar(1.))*p0;
}

template <typename Scalar>
void ControlPolyTwoRK3Tpl<Scalar>::value_inv(double, const Eigen::Ref<const VectorXs>& u, Eigen::Ref<VectorXs> p_out) const{
...
template <typename Scalar>
void ControlPolyTwoRK3Tpl<Scalar>::convert_bounds(const Eigen::Ref<const VectorXs>& u_lb, 
...
template <typename Scalar>
void ControlPolyTwoRK3Tpl<Scalar>::dValue(double t, const Eigen::Ref<const VectorXs>& p, Eigen::Ref<MatrixXs> J_out) const {
  if (static_cast<std::size_t>(p.size()) != np_) {
    throw_pretty("Invalid argument: "
                << "p has wrong dimension (it should be " + std::to_string(np_) + ")");
  }
...
template <typename Scalar>
void ControlPolyTwoRK3Tpl<Scalar>::multiplyByDValue(double t, const Eigen::Ref<const VectorXs>& p, 
...
template <typename Scalar>
void ControlPolyTwoRK3Tpl<Scalar>::multiplyDValueTransposeBy(double t, const Eigen::Ref<const VectorXs>& p, 
    const Eigen::Ref<const MatrixXs>& A, Eigen::Ref<MatrixXs> out) const {
  if (static_cast<std::size_t>(p.size()) != np_) {
    throw_pretty("Invalid argument: "
                << "p has wrong dimension (it should be " + std::to_string(np_) + ")");
